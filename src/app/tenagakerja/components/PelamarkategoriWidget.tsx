...
import {IResponse} from "@/app/api/siloker/pelamarkategori/route";

const ReactECharts = dynamic(() => import('echarts-for-react'), { ssr: false });

export default function PelamarkategoriWidget() {
  const apiUrl = `${getBasePath()}/api/siloker/pelamarkategori`;
  ...
  const pieData = data?.data.map((item) => ({
    ...
    name: truncate(item.kategori.trim(), 18),
  })) || [];
