...
import {IResponse} from "@/app/api/kepegawaian/bypendidikan/route";

const ReactECharts = dynamic(() => import('echarts-for-react'), { ssr: false });

export default function LowonganKotaWidget() {
  const apiUrl = `${getBasePath()}/api/kepegawaian/bypendidikan`;
  const { data, isLoading, isError } = useGetData<IResponse>(apiUrl.toString());

  const truncate = (text: string, max: number) =>
    text.length > max ? text.slice(0, max) + '…' : text;

  const pendidikan = data?.data.map((item) => truncate(item.pendidikan.trim(), 20)) || [];
    ...
    series: [
      {
        ...
        type: 'bar',
        barWidth: '60%',
        data: jumlahPegawai.map((value, index) => ({
          value,
          itemStyle: {
