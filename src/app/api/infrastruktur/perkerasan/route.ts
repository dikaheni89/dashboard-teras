...
export interface PanjangPerTipe {
  panjang: number;
  tipe: string;
}

export interface IResponsePanjangPerTipe {
  status: number;
  success: boolean;
  message: string;
  data: PanjangPerTipe[];
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-pupr/summary/rekap-perkerasan', {
      headers: {
    ...
    const formattedData: PanjangPerTipe[] = data.data.map((item: PanjangPerTipe) => ({
