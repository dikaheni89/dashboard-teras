...
export interface JumlahPenduduk {
  jlh_pddk: number;
  nama_kabkota: string;
}
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-kependudukan/jumlah-pendudukn-per-wilayah', {
      headers: {
    ...
    const formattedData: JumlahPenduduk[] = data.data.map((item: JumlahPenduduk) => ({
