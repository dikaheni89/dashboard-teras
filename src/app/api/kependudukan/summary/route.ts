...
export async function GET() {
  try {
    const responseAPI = await fetch(`${URL_DASHBOAD}teras-kependudukan/summary`, {
      headers: {
    ...
    const formattedData = {
      total_ektp: data.data.total_ektp,
      total_kia: data.data.total_kia,
      total_penduduk: data.data.total_penduduk,
      total_rekam: data.data.total_rekam,
    };
