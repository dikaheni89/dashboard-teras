...
export interface IResponseAktivasi {
  status: number;
  success: boolean;
  message: string;
  data: Aktivasi[];
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-kependudukan/grafik-aktivasi-ikd', {
      headers: {
    ...
    const formattedData: Aktivasi[] = data.data.map((item: Aktivasi) => ({
      aktivasi_ikd: item.aktivasi_ikd,
