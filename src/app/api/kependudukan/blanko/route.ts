...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-kependudukan/sisa-blangko', {
      headers: {
    ...
    const formattedData: JumlahBlanko[] = data.data.map((item: JumlahBlanko) => ({
      sisa_blktp: item.sisa_blktp,
