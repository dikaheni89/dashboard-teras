...
export interface Ktp {
  kemepilikan_ktp: number;
  nama_kabkota: string;
}

export interface IResponseKtp {
  status: number;
  success: boolean;
  message: string;
  data: Ktp[];
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-kependudukan/grafik-ktp', {
      headers: {
    ...
    const formattedData: Ktp[] = data.data.map((item: Ktp) => ({
      kemepilikan_ktp: item.kemepilikan_ktp,
