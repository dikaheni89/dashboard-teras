...
const logError = (message: string, error?: Error) => {
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Implementasi logging production jika diperlukan (contoh: Sentry)
  }
};

export async function GET() {
  try {
    const responseAPI = await fetch(`${URL_DASHBOAD}teras-regulasi/summary`, {
      headers: {
    ...
    const formattedData = {
      total_berlaku: data.data.total_berlaku,
      total_data: data.data.total_data,
      total_tidak_berlaku: data.data.total_tidak_berlaku,
    };
