...
export interface IResponseLowLokasi {
  status: number;
  success: boolean;
  message: string;
  data: {
...
const logError = (message: string, error?: Error) => {
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Sentry.captureException(error); // jika pakai logging eksternal
  }
};

export async function GET() {
  try {
    const responseAPI = await fetch(`${URL_DASHBOAD}teras-pariwisata/low-lokasi`, {
      headers: {
