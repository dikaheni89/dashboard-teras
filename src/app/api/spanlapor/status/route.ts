...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-span-lapor/status-distribution', {
      headers: {
    ...
    const formattedData: BerStatus[] = data.data.map((item: BerStatus) => ({
      status: item.status,
      jumlah: item.jumlah,
    }));
