...
export interface IResponseKategori {
  status: number;
  success: boolean;
  message: string;
  data: LaporanKategori[];
}
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-span-lapor/kategori', {
      headers: {
    ...
    const formattedData: LaporanKategori[] = data.data.map((item: any) => ({
