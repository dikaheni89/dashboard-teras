...
export interface IzinLama {
  jumlah: number;
  kab_kota: string;
}

export interface IResponseLama {
  status: number;
  success: boolean;
  message: string;
  data: IzinLama[];
...
export async function GET() {
  try {
    const responseAPI = await fetch(URL_DASHBOAD + 'teras-investasi/per-kabupaten-data', {
      headers: {
    ...
    const formattedData: IzinLama[] = data.data.map((item: IzinLama) => ({
