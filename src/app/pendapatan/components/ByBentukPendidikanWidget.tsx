...
import {IResponse} from "@/app/api/pendidikan/bybentukpendidikan/route";

const ReactECharts = dynamic(() => import('echarts-for-react'), { ssr: false });

export default function PelamarkategoriWidget() {
  const apiUrl = `${getBasePath()}/api/pendidikan/bybentukpendidikan`;
  ...
  const pieData = data?.data.map((item) => ({
    value: item.jumlah,
    name: truncate(item.bentuk_pendidikan.trim(), 18),
  })) || [];
  ...
  return (
    ...
    >
      <Heading as="h5" size="md" mb={4} px={2}>
        Jumlah Sekolah Berdasarkan Bentuk Pendidikan
      </Heading>
